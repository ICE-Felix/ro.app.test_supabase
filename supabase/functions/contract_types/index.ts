...
import { ContractTypesApiController } from "./controllers/ContractTypesApiController.ts";
import { ContractTypesWebController } from "./controllers/ContractTypesWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new ContractTypesApiController();
    const webController = new ContractTypesWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "contract_types", { type, requestType });
  } catch (error) {
