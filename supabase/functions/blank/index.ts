...
import { BlankApiController } from "./controllers/BlankApiController.ts";
import { BlankWebController } from "./controllers/BlankWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new BlankApiController();
    const webController = new BlankWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "blank", { type, requestType });
  } catch (error) {
