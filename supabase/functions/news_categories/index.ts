...
import { NewsCategoriesApiController } from "./controllers/NewsCategoriesApiController.ts";
import { NewsCategoriesWebController } from "./controllers/NewsCategoriesWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new NewsCategoriesApiController();
    const webController = new NewsCategoriesWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "news_categories", { type, requestType });
  } catch (error) {
