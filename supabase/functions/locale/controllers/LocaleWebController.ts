...
export class LocaleWebController extends Controller<BlankResourceData> {
  // Core Web methods
  override get(id?: string, _req?: Request): Promise<Response> {
    this.logAction('BlankWeb GET', { id });
    
    if (id) {
      console.log(`Web: Fetching blank resource with id: ${id}`);
      // Web-specific resource retrieval logic, possibly returning HTML
      return Promise.resolve(ResponseService.success({ 
        id, 
        data: "Blank Web Resource data", 
        html: "<div>Blank Resource Details</div>" 
      },
      200,
      undefined,
      ResponseType.WEB
