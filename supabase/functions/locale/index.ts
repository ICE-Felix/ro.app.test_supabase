...
import { LocaleApiController } from "./controllers/LocaleApiController.ts";
import { LocaleWebController } from "./controllers/LocaleWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new LocaleApiController();
    const webController = new LocaleWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "locale", { type, requestType });
  } catch (error) {
