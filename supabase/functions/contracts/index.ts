...
import { ContractsApiController } from "./controllers/ContractsApiController.ts";
import { ContractsWebController } from "./controllers/ContractsWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new ContractsApiController();
    const webController = new ContractsWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "contracts", { type, requestType });
  } catch (error) {
