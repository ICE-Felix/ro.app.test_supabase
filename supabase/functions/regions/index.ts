...
import { RegionsApiController } from "./controllers/RegionsApiController.ts";
import { RegionsWebController } from "./controllers/RegionsWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new RegionsApiController();
    const webController = new RegionsWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "regions", { type, requestType });
  } catch (error) {
