...
import { VenueCategoriesApiController } from "./controllers/VenueCategoriesApiController.ts";
import { VenueCategoriesWebController } from "./controllers/VenueCategoriesWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new VenueCategoriesApiController();
    const webController = new VenueCategoriesWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "venue_categories", { type, requestType });
  } catch (error) {
