...
import { UsersApiController } from "./controllers/UsersApiController.ts";
import { UsersWebController } from "./controllers/UsersWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new UsersApiController();
    const webController = new UsersWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "users", { type, requestType });
  } catch (error) {
