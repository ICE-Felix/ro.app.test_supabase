...
import { EventTypesApiController } from "./controllers/EventTypesApiController.ts";
import { EventTypesWebController } from "./controllers/EventTypesWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new EventTypesApiController();
    const webController = new EventTypesWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "event_types", { type, requestType });
  } catch (error) {
