...
import { NewsApiController } from "./controllers/NewsApiController.ts";
import { NewsWebController } from "./controllers/NewsWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new NewsApiController();
    const webController = new NewsWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "news", { type, requestType });
  } catch (error) {
