...
import { PartnersApiController } from "./controllers/PartnersApiController.ts";
import { PartnersWebController } from "./controllers/PartnersWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new PartnersApiController();
    const webController = new PartnersWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "partners", { type, requestType });
  } catch (error) {
