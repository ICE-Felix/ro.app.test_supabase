...
import { NotificationsApiController } from "./controllers/NotificationsApiController.ts";
import { NotificationsWebController } from "./controllers/NotificationsWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new NotificationsApiController();
    const webController = new NotificationsWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "notifications", { type, requestType });
  } catch (error) {
