...
export class BannersClicksController extends Controller<IncrementPayload> {
    private getSupabaseUrl(): string {
        return Deno.env.get("SUPABASE_URL") || "";
    }

    override async get(id?: string, _req?: Request): Promise<Response> {
        this.logAction("BannersClicks GET (increment)", { id });
        ...
        if (!bannerId) {
            return ResponseService.error(
                "Missing banner id",
                "BANNERS_INCREMENT_CLICKS_MISSING_ID",
                400,
                undefined,
                ResponseType.API,
            );
        ...
        try {
            ...
            const updated = await SupabaseBannersService.incrementClicks(client as any, bannerId);
            ...
            return ResponseService.error(
                ...
                "BANNERS_INCREMENT_CLICKS_ERROR",
                400,
    ...
    override async post(data: IncrementPayload, _req?: Request): Promise<Response> {
        this.logAction("BannersClicks POST (increment)", { data });
        const { client } = await AuthenticationService.authenticate(_req!);
        const supabaseUrl = this.getSupabaseUrl();
