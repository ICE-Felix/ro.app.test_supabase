...
import { VenuesApiController } from "./controllers/VenuesApiController.ts";
import { VenuesWebController } from "./controllers/VenuesWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new VenuesApiController();
    const webController = new VenuesWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "venues", { type, requestType });
  } catch (error) {
