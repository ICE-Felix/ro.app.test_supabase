...
import { ShopsApiController } from "./controllers/ShopsApiController.ts";
import { ShopsWebController } from "./controllers/ShopsWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new ShopsApiController();
    const webController = new ShopsWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "shops", { type, requestType });
  } catch (error) {
