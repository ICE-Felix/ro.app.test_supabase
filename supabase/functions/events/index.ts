...
import { EventsApiController } from "./controllers/EventsApiController.ts";
import { EventsWebController } from "./controllers/EventsWebController.ts";
...
export const handler = async (req: Request) => {
  try {
    ...
    const apiController = new EventsApiController();
    const webController = new EventsWebController();

    // Pass the authenticated client, request body, and request type to the route handler
    return await RouteService.handleRequest(req, apiController, webController, "events", { type, requestType });
  } catch (error) {
